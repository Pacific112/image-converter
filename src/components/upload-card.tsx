...
const listImages = async (): Promise<ImageMetadata[]> => {
	const uploadedFiles = await supabase.storage.from('test-uploads').list('', {limit: 20})
	...
	return uploadedFiles.data.map(i => {
		return {
			...
			name: i.name,
			status: 'completed',
			url: supabase.storage.from('test-uploads').getPublicUrl(i.name).data.publicUrl,
		}
	})
}
...
export default function UploadCard() {
	...
	const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		const newFiles = Array.from(event.target.files || []).map(file => {
			...
			supabase.storage.from('test-uploads').upload(imageMetadata.id, file).then(() => console.log("done"))
